...
o	Keep in mind that once the pizza is ordered, no further changes are allowed. We should return the following message if the customer tries to change it: 
...
class PizzaDelivery:
    ...
    def make_order(self) -> str:
        ...
        for ingre, quant in self.ingredients.items():
            output += f" {ingre}: {quant},"
            
        output = output[:-1:] + f" and the price will be {self.price}lv."
