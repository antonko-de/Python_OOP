...
class PhotoAlbum:
    ...
    def add_photo(self, label:str) -> str:
        try:
            next_free_page = [self.photos.index(page) for page in self.photos if len(page) < PhotoAlbum.MAX_PHOTOS_PER_PAGE][0]
            self.photos[next_free_page].append(label)
            return f"{label} photo added successfully on page {next_free_page + 1} slot {len(self.photos[next_free_page])}"
        except IndexError:
    ...
    def display(self):
        ...
        visual = [["[]" for i in row] for row in self.photos]
        output = [mid_row + " ".join(row) +"\n" for row in visual]
        output.append("-----------")
        return ''.join(output)
